...
/*ret*/		Type::getVoidTy(Ctx));
	...
	for(GlobalString *GlobString : GlobalStrings){
		if(GlobString->type == GlobString->SIMPLE_STRING_TYPE){
			...
			Builder.CreateCall(DecodeFunc->getFunctionType(), DecodeFunc, {StrPtr, le});
		}
		else if(GlobString->type == GlobString->STRUCT_STRING_TYPE){
			auto *String = Builder.CreateStructGEP(GlobString->Glob->getValueType(), GlobString->Glob, GlobString->index);
...
Function *createDecodeFunc(Module &M){
 ...
 /*args*/		Type::getInt8PtrTy(Ctx, 8),
	...
	auto *var10 = Builder6->CreateGEP(Type::getInt8PtrTy(Ctx, 8), phi_var7, ConstantInt::get(IntegerType::get(Ctx, 64), 1), "var10");

	auto *var11 = Builder6->CreateLoad(Type::getInt8Ty(Ctx), phi_var7, "var2");
...
void createDecodeStubBlock(Module &M, Function *DecodeStubFunc){
    auto F = Function::Create(FunctionType::get(Type::getVoidTy(M.getContext()),false),GlobalValue::InternalLinkage,"",M);
	auto &Ctx = F->getContext();
	//BasicBlock &EntryBlock = F->getEntryBlock();

	// Create new block
	BasicBlock *NewBB = BasicBlock::Create(Ctx, "DecodeStub", F);
...
vector<GlobalString*> encodeGlobalStrings(Module& M,bool &isCringe){
	int ctr = 0;
	...
	if(ctr > 2)isCringe = true;
	return GlobalStrings;
}

struct StringObfuscatorModPass : public PassInfoMixin<StringObfuscatorModPass> {
	bool &isCringe;
	StringObfuscatorModPass(bool &x): isCringe(x){}
	PreservedAnalyses run(Module &M, ModuleAnalysisManager &MAM) {
	// Transform the strings
	auto GlobalStrings = encodeGlobalStrings(M,isCringe);
